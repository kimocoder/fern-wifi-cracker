from core.fern import tips_window, is_WPS_Device, browse_Reaver_Link, shutil
...
class wpa_attack_dialog(QtWidgets.QDialog,Ui_attack_panel):
    ...
    def __init__(self):
        ...
        if len(self.client_list) == 0:
            _thread.start_new_thread(self.auto_add_clients,())
    ...
    def auto_add_clients(self):
        ...
        while temp_mac_address not in self.client_list:
            ...
            if len(self.client_list) >= 1:
                ...
                if(loop_control):
                    _thread.start_new_thread(self.probe_for_Client_Mac,())
                    loop_control = False
                self.client_update()
                self.update_client_signal.emit()
    ...
    def capture_loop(self):
        ...
        while '1 handshake' not in reader('/tmp/fern-log/WPA-DUMP/capture_status.log'):
            if(self.started == False):                                  # Break deauthentication loop if attack has been stopped
                return
            _thread.start_new_thread(self.deauthenticate_client,())
            time.sleep(10)
            _thread.start_new_thread(self.capture_check,())
        ...
        _thread.start_new_thread(self.launch_brutefore,())
        _thread.start_new_thread(self.wordlist_check,())
        ...
        if self.select_client == str():
            ...
            if not self.settings.setting_exists("wordlist"):
                ...
                if(self.settings.setting_exists(get_temp_name)):                                    # if the line count exists for previously used wordlist
                    ...
                    _thread.start_new_thread(self.find_dictionary_length,(get_temp_name,))           # open thread to count the number of lines in the new wordlist
                ...
                _thread.start_new_thread(self.wpa_capture,())
